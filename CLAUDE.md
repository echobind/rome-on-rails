...
- **Roster control sidecar (optional):** a tiny FastAPI service (`roster-control-sidecar.py`) that lets Roster read/change the agent's LLM model over the tailnet. Backgrounded by `entrypoint.sh` as the `hermes` user via `gosu`, **only** when `ROSTER_CONTROL_TOKEN` is set (fail-closed — no token, or no `gosu`, means it doesn't run; never falls back to root). Binds `127.0.0.1`, exposed to the tailnet via `tailscale serve --http` (plain HTTP; `--tcp` was rejected because it terminates TLS at the tailnet edge and forces clients into `.ts.net` FQDN / cert-verify workarounds — see the 2026-05-15 decision in ARCHITECTURE.md). No new pip deps (FastAPI/uvicorn/PyYAML already in the venv). The interface is governed by `docs/hermes-control-sidecar-contract.md`, a contract co-owned with the Roster repo — change that file first if the interface must move.
